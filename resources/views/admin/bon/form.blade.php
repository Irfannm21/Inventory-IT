<table>
    <tr class="{{ $errors->has('kode') ? 'has-error' : '' }}">
        <td class="px-3">
            <label for="kode">Kode BP</label>
        </td>

        <td class="pt-3">
            <input type="text" name="id" class="form-control"  value="{{old('id') ?? ($result->id ?? '') ?? isset($result->id)}}" hidden>

            <input type="text" id="kode" name="kode" class="form-control"
                value="{{ old('kode', isset($result) ? $result->kode : '') }}" placeholder="Cth: 001/MKT/065">
            @if ($errors->has('kode'))
    ...
    <tr class="{{ $errors->has('tanggal') ? 'has-error' : '' }}">
        ...
        <td class="pt-3">
            <input type="date" id="tanggal" name="tanggal" class="form-control"
                value="{{ old('tanggal', isset($result) ? $result->tanggal : '') }}" placeholder="Cth: 001/MKT/065">
        ...
        <td class="pt-3">
            <select name="departemen" id="departemen" class="form-control">
                ...
                @foreach ($dept as $id => $value)
                    <option value="{{$value->id}}"{{$value->nama == (old('departemen') ?? ($result->bagian->departemen->nama ?? '') ?? isset($result->bagian->departemen->nama)) ? 'selected' : '' }}>{{$value->nama}}</option>
                @endforeach
            </select>
    ...
    <tr class="{{ $errors->has('bagian') ? 'has-error' : '' }}">
        ...
        <td class="pt-3">
            <select name="bagian_id" id="bagian" class="form-control">
                <option value="" selected   >-- Pilih --</option>
                @foreach ($bagian as $id => $value)
                    <option value="{{$value->id}}"{{$value->nama == (old('bagian') ?? ($result->bagian->nama ?? '') ?? isset($result->bagian->nama)) ? 'selected' : '' }}>{{$value->nama}}</option>
                @endforeach
            </select>
    ...
    <tr class="{{ $errors->has('kode') ? 'has-error' : '' }}">
        <td class="px-3">
            <label for="kode">Penerima</label>
        </td>

        <td class="pt-3">
            <input type="text" id="penerima" name="penerima" class="form-control"
                value="{{ old('penerima', isset($result) ? $result->penerima : '') }}" placeholder="Cth: Irfan N">
...
@if(isset($result))
@foreach ($result->detail_bons as $item)
        ...
        <div class="row">
            <div class="col-sm-12 col-md-4">
                <div class="form-group">
                    ...
                    <input type="text" name="barang_id[]" class="form-control"  value="{{old('id') ?? ($item->id ?? '') ?? isset($item->id)}}" hidden>
                    <select name="barang[]" class="form-control" id="">
                        <option value="" selected>-- Pilih --</option>
                    @foreach ($barang as $value)
                        <option value="{{$value->id}}"  {{$value->id == (old('detail_id') ?? ($item->barang_id ?? '') ?? isset($item->barang_id)) ? 'selected' : '' }}>{{$value->nama}}</option>
                    @endforeach
                    </select>
                    @if ($errors->has('nama'))
                        <em class="invalid-feedback">
            ...
            <div class="col col-md-2">
                <div class="form-group {{ $errors->has('satuan[]') ? 'has-error' : '' }}">
                    ...
                    <select name="satuan[]" class="form-control" id="">
                        <option value="" selected>-- Pilih --</option>
                        <option value="Pcs" {{"Pcs" == (old('satuan') ?? ($item->stock->    satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Pcs</option>
                        <option value="Unit" {{"Unit" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Unit</option>
                        <option value="Pack" {{"Pack" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Pack</option>
                        <option value="Dus" {{"Dus" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Dus</option>
                        <option value="Kg" {{"Kg" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Kg</option>
                        <option value="Liter" {{"Liter" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Liter</option>
                        <option value="Meter" {{"Meter" == (old('satuan') ?? ($item->stock->satuan ?? '') ?? isset($item->stock->satuan)) ? 'selected' : '' }}>Meter</option>
                    </select>
