                ...
                <tbody>
                    @foreach($results as $value)
                        <tr data-entry-id="{{ $value->id}}">
                            ...
                            <td>
                                @can('klien_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.kliens.show', $value->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                @can('klien_edit')
                                <a class="btn btn-xs btn-info" href="{{ route("admin.kliens.edit", $value->id)  }}">
                                    {{ trans('global.edit') }}
                                </a>
                                @endcan
                                @can('klien_delete')
                                    <form action="{{ route('admin.kliens.destroy', $value->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
